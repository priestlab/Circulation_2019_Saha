...
def match_codes(key_codes, patient_codes, partial_match=False):
    ...
    if partial_match:
        ...
        if type(key_codes) == dict:
            key_codes = [str(c) for c in key_codes.keys() if str(c) != 'nan']
    ...
    if type(key_codes) == dict:
        return [key_codes[k] for k in (set(key_codes.keys())&set(patient_codes))]
    elif type(key_codes) == list:
        return [k for k in (set(key_codes)&set(patient_codes))]
